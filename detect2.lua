...
function Compkiller:Resizable(Frame: Frame)
	...
	Handle.BackgroundColor3 = Color3.fromRGB(80,80,80)
	Handle.BorderSizePixel = 0
	Handle.ZIndex = (Frame:IsA("GuiObject") and Frame.ZIndex or 1) + 10
	Handle.Parent = Frame
	Handle.Active = true
	...
	UserInputService.InputChanged:Connect(function(input)
		if resizing and input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			local newX = math.max(startSize.X.Offset + delta.X, 150) -- 150 คือความกว้างขั้นต่ำ
			local newY = math.max(startSize.Y.Offset + delta.Y, 100) -- 100 คือความสูงขั้นต่ำ
