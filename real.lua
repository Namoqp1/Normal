...
function Compkiller:Resizable(Frame: Frame)
	...
	UserInputService.InputChanged:Connect(function(input)
		if resizing and input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			local newX = math.max(startSize.X.Offset + delta.X, 150) -- ความกว้างขั้นต่ำ
			local newY = math.max(startSize.Y.Offset + delta.Y, 100) -- ความสูงขั้นต่ำ
